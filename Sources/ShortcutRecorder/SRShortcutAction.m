                       ...
                       context:(void *)aContext
{
    ...
    os_activity_initiate("-[SRShortcutAction observeValueForKeyPath:ofObject:change:context:]", OS_ACTIVITY_FLAG_DEFAULT, ^{
        ...
        if ([newShortcut isKindOfClass:NSDictionary.class])
            ...
            NSError *error = nil;
            newShortcut = [NSKeyedUnarchiver unarchivedObjectOfClass:[SRShortcut class]
                                                         fromData:(NSData *)newShortcut
                                                            error:&error];
            if (error) {
                os_log_error(OS_LOG_DEFAULT, "Failed to unarchive shortcut: %{public}@", error.localizedDescription);
                newShortcut = nil;
            }
...
- (void)_registerHotKeyForShortcutIfNeeded:(SRShortcut *)aShortcut
{
    ...
    {
        os_log_error(OS_LOG_DEFAULT, "Failed to register Carbon hot key: %d - keyCode: %llu, modifierFlags: %llu",
                     error,
                     (uint64_t)aShortcut.keyCode,
                     (uint64_t)aShortcut.modifierFlags);
        return;
    }

    os_log_info(OS_LOG_DEFAULT, "Registered Carbon hot key %u - keyCode: %llu, modifierFlags: %llu",
                hotKeyID.id,
                (uint64_t)aShortcut.keyCode,
                (uint64_t)aShortcut.modifierFlags);
...
- (void)_unregisterHotKeyForShortcutIfNeeded:(SRShortcut *)aShortcut
{
    ...
    if (error != noErr)
    {
        os_log_error(OS_LOG_DEFAULT, "Failed to unregister Carbon hot key %u: %d - keyCode: %llu, modifierFlags: %llu",
                     hotKeyID,
                     error,
                     (uint64_t)aShortcut.keyCode,
                     (uint64_t)aShortcut.modifierFlags);
    ...
    {
        os_log_info(OS_LOG_DEFAULT, "Unregistered Carbon hot key %u - keyCode: %llu, modifierFlags: %llu",
                    hotKeyID,
                    (uint64_t)aShortcut.keyCode,
                    (uint64_t)aShortcut.modifierFlags);
    }
...
+ (NSDictionary<NSString *, id> *)_parseSystemKeyBindings
{
    ...
    if (@available(macOS 10.13, *))
    {
        ...
        {
            os_log_error(OS_LOG_DEFAULT, "Unable to read system key bindings: %{public}@", error.localizedDescription);
            systemKeyBindings = @{};
        }
    }
    else
    {
        ...
        if (!systemKeyBindings)
        {
            os_log_error(OS_LOG_DEFAULT, "Unable to read system key bindings");
            systemKeyBindings = @{};
        }
    }
...
+ (NSDictionary<NSString *, id> *)_parseUserKeyBindings
{
    ...
    if (@available(macOS 10.13, *))
    {
        ...
        if (!userKeyBindings)
        {
            os_log_debug(OS_LOG_DEFAULT, "Unable to read user key bindings: %{public}@", error.localizedDescription);
            userKeyBindings = @{};
        }
    }
    else
